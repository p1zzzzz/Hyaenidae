...
func Login(c *gin.Context) {
	...
	if store.Verify(l.CaptchaId, l.Captcha, true) {
		...
		if err, user := service.Login(u); err != nil {
			global.Hyaenidae_LOG.Error("登陆失败! 用户名不存在或者密码错误!", zap.Any("err", err))
...
func tokenNext(c *gin.Context, user model.SysUser) {
	j := &middleware.JWT{SigningKey: []byte(global.Hyaenidae_CONFIG.JWT.SigningKey)} // 唯一签名
	claims := request.CustomClaims{
		...
		BufferTime:  global.Hyaenidae_CONFIG.JWT.BufferTime, // 缓冲时间1天 缓冲时间内会获得新的token刷新令牌 此时一个用户会存在两个有效令牌 但是前端只留一个 另一个会丢失
		StandardClaims: jwt.StandardClaims{
			NotBefore: time.Now().Unix() - 1000,                                    // 签名生效时间
			ExpiresAt: time.Now().Unix() + global.Hyaenidae_CONFIG.JWT.ExpiresTime, // 过期时间 7天  配置文件
			Issuer:    "qmPlus",                                                    // 签名的发行者
		},
	}
	token, err := j.CreateToken(claims)
	if err != nil {
		global.Hyaenidae_LOG.Error("获取token失败!", zap.Any("err", err))
...
func Register(c *gin.Context) {
	...
	if err != nil {
		global.Hyaenidae_LOG.Error("注册失败!", zap.Any("err", err))
...
func ChangePassword(c *gin.Context) {
	...
	if err, _ := service.ChangePassword(u, user.NewPassword); err != nil {
		global.Hyaenidae_LOG.Error("修改失败!", zap.Any("err", err))
...
func SetUserInfo(c *gin.Context) {
	...
	if err, ReqUser := service.SetUserInfo(user); err != nil {
		global.Hyaenidae_LOG.Error("设置失败!", zap.Any("err", err))
